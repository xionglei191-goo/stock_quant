...
PIPELINE_ARGS=(
  ...
  "--personal-intelligence-symbols" "${AI_QUANT_DAILY_PERSONAL_INTELLIGENCE_SYMBOLS:-AAPL,NVDA,MSFT,300750,600519}"
  "--personal-intelligence-batch-size" "${AI_QUANT_DAILY_PERSONAL_INTELLIGENCE_BATCH_SIZE:-5}"
...
if bool_enabled "$SKIP_PERSONAL_INTELLIGENCE"; then
  PIPELINE_ARGS+=("--skip-personal-intelligence")
fi

if bool_enabled "$RUN_PERSONAL_INTELLIGENCE_EXECUTE"; then
  PIPELINE_ARGS+=("--personal-intelligence-execute")
fi
