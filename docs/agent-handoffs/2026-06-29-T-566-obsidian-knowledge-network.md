...
- Completed:
  ...
  - Default issuer graphs now suppress low-confidence full-graph production-universe bulk `needs_review` position expansion and scope chain nodes to the focus company's position nodes, so a company graph does not become an all-market chain directory unless the user enters an explicit relationship/chain exploration path.
  ...
  - 2026-07-01 community-spread quality gate: `scripts/ui_graph_layout_acceptance.py` now records `community_centroids`, `community_spread_ratio`, and `min_community_spread_ratio`, and supports `--min-community-spread-ratio` so multi-community graph acceptance can fail if communities visually collapse into one center.
- In progress:
  ...
  - 2026-07-01 community-spread acceptance passes with `--min-community-spread-ratio 0.12`, locking the community-target layout against single-center collapse.
- Not started:
...
- Current PostgreSQL AAPL global scope proves the SVG large-graph fallback path: the raw graph can exceed 10k nodes/edges, while the explorer switches to high-performance summary mode and remains within measured frame-time thresholds. Canvas/WebGL is still a measured fallback, not an immediate requirement.
...
1. Keep Canvas/WebGL or virtualization as a follow-up only if `--expect-performance-mode large` fails FPS/frame-time gates on broader sector-scale graphs.
...
- Treat full-graph production-universe `needs_review` chain positions as weak bulk scaffolding in default issuer graphs. They remain traceable and explorable through explicit filters, but they should not dominate the first graph impression or let unrelated ticker chain nodes become the selected focus.
- Use `chain_id:node_id` as the rendered graph identity for industry chain nodes. The API still returns raw `chain_nodes[].node_id`, but the UI and quality center should not split one chain node into both a bare node id and a scoped edge endpoint.
- Use `research_report_id` as the canonical display join between `structured_research_reports` and `report_viewpoints`. Legacy `report_id` and `document_id` remain fallback fields, but the Obsidian graph view should not rely on them for structured report rows.

## SystemService Growth Freeze Review

- New `SystemService` business logic added: limited. Earlier scoped graph-query derivation was inside `query_graph`; this continuation moved ownership/holder/industry relationship planning into `app/service_modules/graph_derived_relationships.py` and kept `SystemService` as the node/edge assembly facade. The latest chain-node canonicalization, structured-research display mapping, community-target layout correction, and community-spread acceptance gate did not add `SystemService` behavior; they touched the UI graph model, quality-center structure metrics, browser acceptance, tests, API documentation, and roadmap/handoff records.
...
- The graph is closer to Obsidian behavior but not complete: sector-scale graphs still need threshold-driven evaluation beyond the current AAPL large-mode proof.
- Current overlap and community-spread checks are approximate but now repeatable via `scripts/ui_graph_layout_acceptance.py`.
- The graph remains SVG-based; it now has a measured high-performance summary mode, but if broader sector-scale graphs fail the FPS/frame thresholds, performance may require Canvas/WebGL.
...
- `artifacts/obsidian-knowledge-graph-seed-community-spread-55685.json`: local-only current-code Obsidian seed result used for community-spread acceptance, not production-grade evidence.
- `artifacts/ui-graph-layout-community-spread-55685.json`: local-only headless Chrome graph layout acceptance with `--min-community-spread-ratio`, not production-grade evidence.
...
- `artifacts/obsidian-knowledge-graph-seed-research-layer-55664.json`: local-only current-code Obsidian seed result used for structured research display acceptance, not production-grade evidence.
- `artifacts/ui-graph-layout-structured-research-55664.json`: local-only browser acceptance proving `structured_research_reports` render as visible `research` graph nodes and `report_viewpoints` link through `research_report_id`, not production-grade evidence.
...
- `python3 -m unittest discover -s tests`: passed, 320 tests.
- `python3 scripts/security_check.py .`: passed, 0 findings across 360 checked files.
- `python3 scripts/check_handoffs.py`: passed.
- `git diff --check`: passed.
- `python3 -m unittest tests.test_system.SystemServiceTests.test_graph_quality_center_structure_uses_canonical_chain_node_ids tests.test_system.SystemServiceTests.test_graph_quality_center_structure_uses_display_market_data_model tests.test_system.SystemServiceTests.test_query_graph_scopes_company_positions_to_focus_issuer tests.test_system.SystemServiceTests.test_graph_quality_center_reports_gaps_and_actions`: passed after chain-node canonicalization.
- `python3 scripts/ui_static_check.py && python3 -m py_compile app/service_modules/graph_quality_center.py app/service_modules/graph_intelligence.py scripts/ui_graph_layout_acceptance.py tests/test_system.py`: passed after chain-node canonicalization.
- `python3 scripts/ui_graph_layout_acceptance.py http://127.0.0.1:55662 --symbol AAPL --scope global --min-nodes 20 --min-links 30 --min-community-labels 2 --max-overlap-pairs 80 --max-near-edge-nodes 2 --expect-performance-mode standard --max-chain-node-splits 0 --output artifacts/ui-graph-layout-canonical-chain-nodes-55662.json --timeout 60`: passed with `chain_node_splits=[]`, 21 visible nodes, 44 visible links, 29 full nodes, 52 full links, 4 community labels, 0 overlap pairs, 0 near-edge nodes, and about 60 FPS.
- `.venv/bin/python scripts/ui_graph_layout_acceptance.py http://127.0.0.1:55664 --symbol AAPL --scope local --min-nodes 26 --min-links 60 --min-community-labels 3 --min-visible-communities 3 --min-industry-nodes 5 --min-raw-knowledge-nodes 5 --min-raw-structured-reports 1 --min-visible-knowledge-types 2 --max-overlap-pairs 14 --max-near-edge-nodes 2 --forbid-filter-chip "证券:" --max-chain-node-splits 0 --output artifacts/ui-graph-layout-structured-research-55664.json --timeout 60`: passed with `raw_structured_reports=1`, visible node type `research`, 28 visible nodes, 83 visible links, 4 community labels, 5 industry nodes, 0 overlap pairs, 0 near-edge nodes, and `chain_node_splits=[]`.
- Direct current-code API probe on `http://127.0.0.1:55664/api/graph/query?issuer_id=issuer_aapl&security_id=security_aapl_us&market_data_limit=20` confirmed one `structured_research_reports` row (`srr_obsidian_aapl_ai_device`), one `report_viewpoints` row linked by `research_report_id`, and research edges including `COVERED_BY_REPORT`, `REPORT_HAS_VIEWPOINT`, and `VIEWPOINT_ON_COMPANY`.
- `python3 -m unittest tests.test_system.SystemServiceTests.test_company_intelligence_first_class_models_are_exposed_and_aggregated tests.test_system.SystemServiceTests.test_query_graph_scopes_company_positions_to_focus_issuer tests.test_system.SystemServiceTests.test_graph_derived_relationships_plans_display_edges tests.test_system.SystemServiceTests.test_obsidian_knowledge_graph_seed_creates_multi_dimension_network tests.test_system.SystemServiceTests.test_graph_acceptance_fixture_supports_industry_relationship_filters`: passed after the default full-graph noise fix.
- Direct current-code API probes on `http://127.0.0.1:55661` against local PostgreSQL passed: default `issuer_aapl` returned 1 issuer, 1 company position, 1 chain node, 54 edges, and no AAMI chain node; explicit `relationship_type=industry_peer` kept the same scoped production-universe focus node because no eligible real peer exists in the current local DB.
- `python3 scripts/ui_graph_layout_acceptance.py http://127.0.0.1:55661 --symbol AAPL --scope global --min-nodes 20 --min-links 30 --min-community-labels 2 --max-overlap-pairs 80 --max-near-edge-nodes 2 --expect-performance-mode standard --output artifacts/ui-graph-layout-scoped-55661.json --timeout 60`: passed with 22 visible nodes, 44 visible links, 30 full nodes, 52 full links, 4 community labels, 0 overlap pairs, 0 near-edge nodes, standard render mode, and no raw label leaks.
...
- `python3 scripts/ui_graph_layout_acceptance.py http://127.0.0.1:55659 --symbol AAPL --scope global --min-nodes 35 --min-links 45 --min-community-labels 2 --max-overlap-pairs 100 --max-near-edge-nodes 2 --expect-performance-mode large --output artifacts/ui-graph-layout-performance-mode-55659.json --timeout 60`: passed. It measured raw graph `10872` nodes / `10894` links, visible graph 47 nodes / 48 links, `performance_mode=large`, `graph_stage_performance_mode=true`, 3 community labels, 0 raw visible text leaks, 0 near-edge nodes, saved-subgraph restore passing, and average frame time 6.5ms.
- `python3 scripts/seed_obsidian_knowledge_graph.py http://127.0.0.1:55685 --output artifacts/obsidian-knowledge-graph-seed-community-spread-55685.json --timeout 30`: passed and created 49 local Obsidian seed records on an isolated current-code SQLite service.
- `python3 scripts/ui_graph_layout_acceptance.py http://127.0.0.1:55685 --symbol AAPL --scope local --min-nodes 26 --min-links 60 --min-community-labels 3 --min-visible-communities 3 --min-community-spread-ratio 0.12 --min-industry-nodes 5 --min-raw-knowledge-nodes 5 --min-visible-knowledge-types 2 --max-overlap-pairs 14 --max-near-edge-nodes 2 --forbid-filter-chip "证券:" --check-focus-switch --output artifacts/ui-graph-layout-community-spread-55685.json --timeout 90`: passed. It measured 28 DOM nodes, 83 links, 4 visible communities, community spread ratio `0.5686`, minimum pair ratio `0.2079`, 5 industry nodes, 0 overlap pairs, 0 near-edge nodes, no raw text leaks, reversible focus switching, and about 60 FPS / 1.1ms average frame time.
...
9. Evaluate Canvas/WebGL only if broader sector-scale graphs fail measured FPS/frame thresholds in large performance mode.
