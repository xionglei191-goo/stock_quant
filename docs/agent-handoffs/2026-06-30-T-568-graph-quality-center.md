...
- Path-panel next hops now use `graphPathNextHops()` instead of taking the first three adjacent links. The helper keeps candidates inside the visible node map, deduplicates by node, and sorts by graph node priority, knowledge/research/event/evidence value, and link weight so the side branches in a highlighted path remain useful exploration entry points.
