...
- Browser layout acceptance now supports `--max-visible-nodes`, `--max-visible-links`, and `--max-link-label-dom-count`. Large-graph runs can pair those with `--expect-performance-mode large` to prove the SVG explorer is still rendering a bounded visible subgraph instead of dumping the full backend graph into the DOM. In large mode the front end only creates edge-label DOM for focus/selected/path-related links, path refresh triggers a redraw so newly highlighted path links still get readable labels, and `renderStats.linkLabelDomCount` is the shared runtime/artifact metric.
