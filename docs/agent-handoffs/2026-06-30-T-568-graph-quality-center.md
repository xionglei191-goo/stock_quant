...
- `renderKnowledgeGraphExplorer()` now maintains a shared `knowledgeGraphState.nodeById` map for the visible layout. Edge markup, path/trail/history pruning, inspector neighbor rendering, label updates, SVG position sync, centering, and drag interactions reuse O(1) lookups instead of repeatedly scanning `knowledgeGraphState.nodes`; the node `<g>` transform update uses the same Map so dynamic layout and dragging keep nodes, edges, and edge labels aligned.
- Path-panel next hops now use `graphPathNextHops()` instead of taking the first three adjacent links. The helper keeps candidates inside the visible node map, deduplicates by node, and sorts by graph node priority, knowledge/research/event/evidence value, link weight, and the current path node's community context so the side branches in a highlighted path remain useful exploration entry points.
- Graph markup now escapes SVG edge ids/endpoints, edge labels, node ids/labels, community keys/labels/quality text, path-step labels, and inspector neighbor labels/ids before writing `innerHTML`, keeping special characters or raw source text from breaking visible graph labels and interaction attributes.
