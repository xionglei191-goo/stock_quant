...
- `renderKnowledgeGraphExplorer()` now builds a `layoutNodeById` map before edge markup generation, so visible edge rendering does endpoint lookup in O(1) instead of repeatedly scanning `layout.nodes`.
