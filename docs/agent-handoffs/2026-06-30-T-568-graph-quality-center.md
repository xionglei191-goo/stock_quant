...
- SVG graph markup now escapes edge ids/endpoints, edge labels, node ids/labels, and community keys/labels/quality text before writing `innerHTML`, keeping special characters or raw source text from breaking visible graph labels and interaction attributes.
