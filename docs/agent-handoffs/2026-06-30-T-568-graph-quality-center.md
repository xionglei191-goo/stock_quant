...
- Browser layout acceptance now supports `--max-visible-nodes` and `--max-visible-links`. Large-graph runs can pair those with `--expect-performance-mode large` to prove the SVG explorer is still rendering a bounded visible subgraph instead of dumping the full backend graph into the DOM.
