...
- New `SystemService` business logic added: no; this pass moved existing industry relationship, 13F same-holder, and ownership-holder derivation planning out of `query_graph` into a domain module, tightened planner outputs to dataclass contracts, and kept `query_graph` as the compatibility assembly surface.
- Domain module used: yes, `app/service_modules/graph_quality_center.py` and `app/service_modules/graph_derived_relationships.py`.
- Facade behavior protected by: focused tests for API gap/action output, enrichment dry-run no-write behavior, CLI artifact writing, industry relationship graph-query regressions, chain-node canonical structure metrics, 13F holder-key graph-query regressions, and ownership holder-key browser regression.
