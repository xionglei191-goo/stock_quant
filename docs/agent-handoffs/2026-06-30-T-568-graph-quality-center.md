...
- `renderKnowledgeGraphExplorer()` now maintains a shared `knowledgeGraphState.nodeById` map for the visible layout. Edge markup, focus-label lookup, path/trail/history pruning, inspector neighbor rendering, label updates, SVG position sync, centering, and drag interactions reuse O(1) lookups instead of repeatedly scanning `knowledgeGraphState.nodes`; the node `<g>` transform update uses the same Map so dynamic layout and dragging keep nodes, edges, and edge labels aligned.
