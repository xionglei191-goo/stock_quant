...
- `renderKnowledgeGraphExplorer()` now maintains a shared `knowledgeGraphState.nodeById` map for the visible layout. Edge markup, path/trail/history pruning, inspector neighbor rendering, label updates, centering, and drag interactions reuse O(1) lookups instead of repeatedly scanning `knowledgeGraphState.nodes`.
