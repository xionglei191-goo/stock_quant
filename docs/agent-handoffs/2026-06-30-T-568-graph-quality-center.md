...
- Graph exploration views are now shareable. The “复制视图链接” control writes a `#graph=` URL state containing issuer/security, focus node, selected node, local/global scope, depth, search, path anchors, trail, focus history, expanded nodes, and transform. Opening `/ui#graph=...` restores the pending graph state, seeds the issuer input, loads the company intelligence page, and shows `已从分享链接恢复视图` after the graph is rendered.
- Browser layout acceptance now supports `--check-share-view`. The gate generates a share URL in the real UI, decodes the URL payload, clears volatile graph state, reapplies the shared state, rerenders, and fails unless focus, selection, filters, search, trail, hash, and restore status all round-trip.
- Visible graph edges now have directional encoding. Each rendered SVG line uses `graphArrowMarker` and carries `data-source`, `data-target`, `data-direction-label`, and `aria-label` in the form `<source> -> <target> · <relationship label>`, so relationship direction is readable from the graph itself instead of only from side tables or inspector metadata.
- Browser layout acceptance now supports `--expect-directed-edges`. The gate checks that every visible edge has the arrow marker plus source/target and a direction label.
- Visible graph edges now also encode strength. `graphLinkStrength()` combines existing link weight with optional confidence/score/weight metadata, writes `data-weight`, `data-confidence`, and `data-strength`, applies inline `stroke-width`, and adds readable strength/confidence text to the edge direction label.
- Browser layout acceptance now supports `--expect-weighted-edges`. The gate checks that every visible edge has a positive strength/stroke-width encoding and that multi-edge graphs show more than one strength bucket.
- Visible graph edges are now first-class selectable controls. The SVG line has `role="button"`, `tabindex="0"`, and keyboard activation, selection uses a cached `linkById` map, and the inspector switches to a relationship view with edge label, strength, confidence, weight, source/target endpoint buttons, and sanitized relationship trace.
- Browser layout acceptance now supports `--check-link-selection`. The gate clicks a real visible edge and fails unless `selectedLinkId` is set, node selection is cleared, `.is-selected-link` is applied, the edge is keyboard-focusable, relationship metadata includes strength, and both endpoint buttons render.
...
- API/storage/UI/paper-only impact: one new quality-center API endpoint from the original T-568 work; no storage schema change; UI display contract now explicitly includes structured research report/viewpoint linkage from existing `/api/graph/query` fields, a read-only graph display-quality gate, dry-run remediation routing for failed display gates, community-balance structure fields in `quality_gate.structure`, a front-end display row for community balance, large-graph edge-label DOM reduction, shareable graph view URLs, visible edge direction/strength encoding, first-class relationship-edge selection in the graph inspector, and browser acceptance artifacts that surface seed dependency, bounded visible-DOM budgets, share-view round-trips, directed-edge rendering, weighted-edge rendering, and clickable relationship-edge inspection; no broker or live-trading behavior.
...
- Current-code shareable graph view acceptance on isolated SQLite service:
  - Service: `AI_QUANT_PORT=55705 AI_QUANT_POSTGRES_DSN= AI_QUANT_DATABASE_URL= AI_QUANT_DB=/tmp/ai_quant_graph_share_state.db AI_QUANT_OBJECT_STORE_BACKEND=local AI_QUANT_OBJECT_STORE=/tmp/ai_quant_graph_share_objects AI_QUANT_SEARCH_BACKEND=local python3 -m app.server`
  - Fixture: `python3 scripts/graph_acceptance_fixture.py http://127.0.0.1:55705`
  - Browser graph acceptance: `python3 scripts/ui_graph_layout_acceptance.py http://127.0.0.1:55705 --symbol AAPL --scope local --min-nodes 21 --min-links 49 --min-community-labels 3 --min-visible-communities 3 --min-community-spread-ratio 0.12 --min-industry-nodes 4 --min-visible-knowledge-types 1 --max-overlap-pairs 14 --max-near-edge-nodes 2 --forbid-filter-chip "证券:" --check-share-view --skip-readiness-probe --output artifacts/ui-graph-layout-share-view-55705.json --timeout 60`
  - Result: passed with `failure_count=0`. The share-view gate confirmed `url_has_graph_hash=true`, decoded/restored focus and selected node `rel_graph_acceptance_aapl_coverage_1`, restored `scope=local`, `depth=3`, `search=share-gate`, `restored_trail_nodes=2`, and status `已从分享链接恢复视图`.
  - Note: this artifact is a local-only UI behavior acceptance over the lightweight graph acceptance fixture. It proves share-state round-trip behavior, not production-grade graph data density.
- Current-code directed-edge graph rendering acceptance on isolated SQLite service:
  - Service: `AI_QUANT_PORT=55706 AI_QUANT_POSTGRES_DSN= AI_QUANT_DATABASE_URL= AI_QUANT_DB=/tmp/ai_quant_graph_directed_edges_state.db AI_QUANT_OBJECT_STORE_BACKEND=local AI_QUANT_OBJECT_STORE=/tmp/ai_quant_graph_directed_edges_objects AI_QUANT_SEARCH_BACKEND=local python3 -m app.server`
  - Fixture: `python3 scripts/graph_acceptance_fixture.py http://127.0.0.1:55706 --output artifacts/graph-acceptance-fixture-directed-edges-55706.json --timeout 60`
  - Browser graph acceptance: `python3 scripts/ui_graph_layout_acceptance.py http://127.0.0.1:55706 --symbol AAPL --scope local --min-nodes 21 --min-links 49 --min-community-labels 3 --min-visible-communities 3 --min-community-spread-ratio 0.12 --min-industry-nodes 4 --min-visible-knowledge-types 1 --max-overlap-pairs 14 --max-near-edge-nodes 2 --forbid-filter-chip "证券:" --expect-directed-edges --skip-readiness-probe --output artifacts/ui-graph-layout-directed-edges-55706.json --timeout 60`
  - Result: passed with `failure_count=0`, `directed_link_count=49`, `links=49`, and `missing_directed_links=[]`, proving every visible edge had arrow marker plus direction metadata.
  - Note: this artifact is a local-only UI rendering acceptance over the lightweight graph acceptance fixture. It proves visual direction encoding, not production-grade graph data density.
- Current-code weighted-edge graph rendering acceptance on isolated SQLite service:
  - Service: `AI_QUANT_PORT=55707 AI_QUANT_POSTGRES_DSN= AI_QUANT_DATABASE_URL= AI_QUANT_DB=/tmp/ai_quant_graph_weighted_edges_state.db AI_QUANT_OBJECT_STORE_BACKEND=local AI_QUANT_OBJECT_STORE=/tmp/ai_quant_graph_weighted_edges_objects AI_QUANT_SEARCH_BACKEND=local python3 -m app.server`
  - Fixture: `python3 scripts/graph_acceptance_fixture.py http://127.0.0.1:55707 --output artifacts/graph-acceptance-fixture-weighted-edges-55707.json --timeout 60`
  - Browser graph acceptance: `python3 scripts/ui_graph_layout_acceptance.py http://127.0.0.1:55707 --symbol AAPL --scope local --min-nodes 21 --min-links 49 --min-community-labels 3 --min-visible-communities 3 --min-community-spread-ratio 0.12 --min-industry-nodes 4 --min-visible-knowledge-types 1 --max-overlap-pairs 14 --max-near-edge-nodes 2 --forbid-filter-chip "证券:" --expect-directed-edges --expect-weighted-edges --skip-readiness-probe --output artifacts/ui-graph-layout-weighted-edges-55707.json --timeout 60`
  - Result: passed with `failure_count=0`, `weighted_link_count=49`, `links=49`, `missing_weighted_links=[]`, and `15` edge strength buckets, proving every visible edge had a positive stroke-width strength encoding and readable strength metadata.
  - Note: this artifact is a local-only UI rendering acceptance over the lightweight graph acceptance fixture. It proves visual strength encoding, not production-grade graph data density.
