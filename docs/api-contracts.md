...
- `min_edges`、`min_communities`、`min_layers`、`min_structural_nodes`、`max_hub_edge_share`、`max_leaf_ratio`、`min_largest_component_ratio`、`max_community_node_share`：可选；覆盖质量门结构阈值。`max_community_node_share` 默认 `0.72`，用于拒绝一个社区吞掉绝大多数展示节点的失衡图谱。
