...
- 非本机证据状态面板：`scripts/production_evidence_status_board.py` 已生成 `docs/production-evidence-status-board.md`，当前 6 个 owner、17 个外部证据任务、80 个 artifact 字段均为 placeholder URI，状态为 `waiting_for_external_evidence`；只有全部替换为真实 staging/production URI、通过 artifact inventory 和 strict release gate 后，才允许把对应 BLOCKED 任务改为 DONE。
