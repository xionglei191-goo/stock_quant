...
- `DONE` T-566 Obsidian 标准可探索知识网络升级
  ...
  - 当前验证（图谱输出转义）：图谱 SVG 的边 id/source/target、边标签、节点 id/label、社区 key/label/quality，以及路径步骤和邻居列表中的节点/边文本、交互 `data-node-id` 统一走 `escapeHtml()` 后再写入 `innerHTML`，防止特殊字符或 raw 文本破坏节点/边/社区标签和路径/邻居交互。
