...
- `DONE` T-566 Obsidian 标准可探索知识网络升级
  ...
  - 当前验证（渲染热路径降债）：`renderKnowledgeGraphExplorer()` 现在把可见节点索引维护为共享 `knowledgeGraphState.nodeById` Map；边渲染端点、focus label、path/trail/history、inspector 邻居、标签更新、动画位置同步、居中和拖拽都复用 O(1) 查找，避免大图 SVG 渲染和交互路径反复扫描 `knowledgeGraphState.nodes`，并确保动态布局/拖拽时节点 `<g>` 与边/边标签同步移动。
