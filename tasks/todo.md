...
- `DONE` T-566 Obsidian 标准可探索知识网络升级
  ...
  - 当前验证（焦点路径上下文门禁）：`scripts/ui_graph_layout_acceptance.py --check-focus-switch` 现在记录并校验 `path_after_focus` / `path_after_back_focus`，要求前进切焦点后路径起点为新焦点、终点为上一焦点，返回焦点后反向保留来路，并且两次切换后都有可见 path node / path link 高亮；同一门禁还点击 `.graph-neighbor[data-node-id]`，要求侧栏邻居点击后 `focusId` / `selectedId` 都切到该邻居节点，防止路径上下文和侧栏漫游能力只停留在静态测试。
