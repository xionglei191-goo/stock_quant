...
- `DONE` T-566 Obsidian 标准可探索知识网络升级
  ...
  - 当前验证（大图 DOM 预算门）：`scripts/ui_graph_layout_acceptance.py` 新增 `--max-visible-nodes` / `--max-visible-links`，可在 `--expect-performance-mode large` 场景强制可见 SVG 节点/边不超过预算，并把失败时的 `render_stats` 写进 artifact，避免全局大图回退成把上千节点/边直接塞进 DOM。
