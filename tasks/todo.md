...
- `DONE` T-566 Obsidian 标准可探索知识网络升级
  ...
  - 当前验证（路径旁支阅读质量）：路径面板 next-hop 从“当前节点前 3 条边”改为 `graphPathNextHops()`，只展示当前可见节点，并按知识节点/研究/事件/证据优先级、边权和语义价值排序；跨社区旁支判断使用当前路径节点社区作为上下文，而不是隐式焦点节点，浏览器验收 artifact 记录 `next_hop_node_types`；前端高亮状态集中到 `knowledgeGraphNodeHighlightState()` / `knowledgeGraphLinkHighlightState()`，路径节点/路径边在同时选中某个节点时不再被选中态压暗，避免多跳路径阅读入口退回低价值、不可见或不可读旁支。
