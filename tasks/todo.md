...
- `DONE` T-566 Obsidian 标准可探索知识网络升级
  ...
  - 当前验证（大图 DOM 预算门）：`scripts/ui_graph_layout_acceptance.py` 新增 `--max-visible-nodes` / `--max-visible-links` / `--max-link-label-dom-count`，可在 `--expect-performance-mode large` 场景强制可见 SVG 节点、边和边标签 DOM 不超过预算，并把失败时的 `render_stats` 写进 artifact；前端大图模式只为焦点/选中/路径相关边创建 `<text class="graph-link-label">`，避免全局大图回退成把上千节点/边/标签直接塞进 DOM。
