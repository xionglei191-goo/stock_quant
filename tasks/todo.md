...
- `DONE` T-614 首批全量研报 clone 重解析演练
  ...
  - 执行结果：人工批准精确绑定 manifest/batch SHA；独立 clone attestation 证明数据库、容器、内部网络、只读 raw、只读根文件系统、本地 object/search 后端和主服务不可达。run1/run2 均为 250/250 成功、247 `text_indexed`、3 `needs_text_review`、1937 citation evidence；run1 `ingest_created=250`，run2 `ingest_created=0`，250 个 report/document/content/status/evidence/manual-review 逐项一致且 mismatch 为 0。
  - 容量/耗时：run1 2144.972 秒，run2 2705.687 秒；clone 数据库从 15502007319 bytes 增至 run1 的 15541189655 bytes，再增至 run2 的 15564430359 bytes。run2 逻辑 records 保持 46197，仅 audit 从 36140 增至 36394，物理增量主要是 MVCC/audit；其余 43 批串行双跑机械外推约 57.9 小时，不构成执行授权。
...
- `DOING` T-617 persistent clone 累计状态与调度静默架构
  ...
  - 当前进展：batch 0004、0005 已在精确批准的独立 clone 中完成双跑。batch 0005 为 250/250 成功，245 `text_indexed`、5 `manual_review`、1870 citation evidence，run2 `ingest_created=0` 且逐项一致。最终 clone `34695/36142/28365474`，数据库 `15511968791` bytes；备份 restore equality 已验证并保留至 2026-07-28，clone 资源已清理，主库健康与基线计数未变。segment state contract 已绑定 clone identity、plan/manifest、run1/run2、prior-run SHA、九项累计计数和 checkpoint/abort/resume；实际 executor 已接入 active state 校验、run2 checkpoint 写入、30 分钟 freshness、scheduler stopped、writer container stopped、writer sessions=0、primary unreachable 的 quiescence proof 门禁，并提供直接只读观测命令。`.venv` 下 full CI 544 tests 及 UI/安全/文档检查已通过。batch 0006 只读 preflight 已生成但仍被人工批准、独立 attestation 和 quiescent window 阻断；当前 `ai-quant-daily-update.timer` 仍 active，batch 0006-0044 不执行。
...
- T-566 历史执行日志（最终状态与剩余边界见下方权威 T-566 条目）
  ...
  - 当前验证（社区目标布局修正）：`tickKnowledgeGraphLayout()` 的非焦点节点目标从“焦点/画布中心中点”抽为 `graphLayoutTargetForNode()`，全局和知识/展开节点优先回到社区中心，局部普通节点也按社区中心与画布中心混合锚定，减少公司焦点对全图的持续吸附；55684 当前代码 + Obsidian seed 浏览器验收通过，28 DOM 节点 / 83 边、4 个社区标签、4 个可见社区、5 个产业节点、0 重叠、0 贴边、raw 文本泄漏 0、焦点切换和保存子图通过，artifact 为 `artifacts/ui-graph-layout-community-targets-55684.json`。
  - 当前验证（社区分散度质量门）：`scripts/ui_graph_layout_acceptance.py` 新增 `--min-community-spread-ratio`，按浏览器实测节点坐标聚合 `community_centroids`，用社区质心平均距离 / 画布短边衡量社区是否真正分散，避免只靠节点数、边数、社区数通过却视觉上仍坍缩到单中心。
  - 当前验证（质量中心社区均衡门）：`/api/graph/quality-center` 的展示结构新增 `community_counts` / `max_community_node_share`，默认 `max_community_node_share=0.72`，用 `community_balance` 失败项识别“社区数达标但一个社区吞掉多数展示节点”的图谱；`scripts/graph_quality_center.py --browser-matrix` 可同时透传浏览器社区分散度阈值。
  - 当前验证（社区均衡 UI 展示）：知识图谱页展示质量面板新增 `社区节点失衡` 行，显示 `max_community_node_share` 实际/上限百分比和 `community_counts` 可读摘要，避免质量中心发现社区失衡但前端只显示泛化结构失败。
  - 当前验证（seed 依赖进入浏览器验收）：`scripts/ui_graph_layout_acceptance.py` 默认把 `/api/graph/knowledge-network/readiness` 摘要写入 artifact 的 `knowledge_network_readiness`，记录 `seed_dependency`、缺失/稀薄层、跨层链接和 no-broker 边界；生产级展示验收可加 `--require-non-seed-readiness`，在 `seed_dependent=true` 或 readiness 未 ready 时直接失败，避免 seed/fixture 图谱画得好却被误当成真实数据完成。
  - 当前验证（渲染热路径降债）：`renderKnowledgeGraphExplorer()` 现在把可见节点索引维护为共享 `knowledgeGraphState.nodeById` Map，并缓存 `communitySummaries` / `communityNodesByKey`；边渲染端点、focus label、社区标签更新/点击代表节点、path/trail/history、inspector 邻居、标签更新、动画位置同步、居中和拖拽都复用预构建索引，避免大图 SVG 渲染和交互路径反复扫描 `knowledgeGraphState.nodes` 或重复计算社区摘要；动态布局 tick 也复用共享 Map 查找边端点和本地锚点，不再每帧从节点数组重建对象索引；动态布局/拖拽时节点 `<g>` 与边/边标签同步移动，社区标签只从已缓存的社区节点分组轻量刷新坐标，避免摘要静态缓存导致标签位置滞后。
  - 当前验证（路径旁支阅读质量）：路径面板 next-hop 从“当前节点前 3 条边”改为 `graphPathNextHops()`，只展示当前可见节点，并按知识节点/研究/事件/证据优先级、边权和语义价值排序；跨社区旁支判断使用当前路径节点社区作为上下文，而不是隐式焦点节点，浏览器验收 artifact 记录 `next_hop_node_types`；前端高亮状态集中到 `knowledgeGraphNodeHighlightState()` / `knowledgeGraphLinkHighlightState()`，路径节点/路径边在同时选中某个节点时不再被选中态压暗；侧栏邻居、路径 next-hop、焦点历史和探索轨迹统一通过 `activateKnowledgeGraphSideNode()` 激活；焦点切换、显式聚焦和返回焦点统一通过 `updateKnowledgeGraphFocusPathAnchor()` 保留“新焦点 -> 上一焦点”的来路路径，避免多跳路径阅读入口退回低价值、不可见、不可读、无点击响应或切焦点即丢失上下文的旁支。
  - 当前验证（焦点路径上下文门禁）：`scripts/ui_graph_layout_acceptance.py --check-focus-switch` 现在记录并校验 `path_after_focus` / `path_after_back_focus`，要求前进切焦点后路径起点为新焦点、终点为上一焦点，返回焦点后反向保留来路，并且两次切换后都有可见 path node / path link 高亮；同一门禁还点击 `.graph-neighbor[data-node-id]`，要求侧栏邻居点击后 `focusId` / `selectedId` 都切到该邻居节点；侧栏邻居、路径 next-hop、焦点历史和探索轨迹入口都使用显式 `button type="button"` 与动作化 `aria-label`，邻居入口提供 `:focus-visible` 焦点样式，防止路径上下文和侧栏漫游能力只停留在静态测试或鼠标专用交互。
  - 当前验证（主 SVG 键盘可达）：图谱主画布里的节点 `<g>` 和社区标签 `<g>` 现在都有 `role="button"`、`tabindex="0"` 和动作化 `aria-label`；Enter/Space 会触发与鼠标一致的节点选择/社区代表节点切焦点；节点和社区标签都有 `:focus-visible` 可视焦点样式，避免核心图谱探索只能依赖 pointer 操作或键盘焦点不可见。
  - 当前验证（主 SVG 键盘浏览器门禁）：`scripts/ui_graph_layout_acceptance.py --check-focus-switch` 现在会在真实浏览器 DOM 中聚焦 `.graph-node-svg` 并发送 Enter，要求焦点和选中都切到该 SVG 节点，同时检查 `role/button`、`tabindex`、`aria-label` 和 `:focus-visible`；同一门禁还会聚焦 `.graph-community-label` 并发送 Space，要求焦点切到该社区代表节点，避免键盘可达性只停留在静态代码断言。
  - 当前验证（主 SVG 键盘浏览器复验）：当前代码在 `AI_QUANT_PORT=55701` 隔离 SQLite 服务 seed 49 条 Obsidian 样本后，AAPL local layout acceptance 通过，artifact `artifacts/ui-graph-layout-keyboard-focus-55701.json`，`failure_count=0`；`focus_switch.keyboard_node` 证明 SVG 节点 Enter 后 `focusId/selectedId` 均切到 `pos_obsidian_aapl_edge_device` 且 `focus_visible_before_key=true`，`community_click.keyboard_community` 证明社区标签 Space 后切到产业链代表节点。同步修复 pointer 链路后 click 二次触发 `selectKnowledgeGraphNode()` 清空来路路径的问题，并把 `pathStartId/pathEndId` 作为裁剪保留锚点，保证新焦点到上一焦点的路径高亮不被局部裁剪丢弃。
  - 当前验证（移动端图谱门禁）：图谱移动端布局在 `max-width:900px` 下改为单列，主图高度降为 480px、详情面板最小高度降为 360px，工具栏/视图控件允许换行；`scripts/ui_graph_layout_acceptance.py` 新增 `--viewport-width/height`、`--expect-mobile-layout` 和 `responsive_layout` / `initial_graph` artifact 指标。当前代码在 `AI_QUANT_PORT=55702` 隔离 SQLite 服务 seed 后，AAPL 430px 移动视口验收通过，artifact `artifacts/ui-graph-layout-mobile-55702.json`，初始图 28 节点 / 79 边 / 4 个社区标签，`stage_overflows_viewport=false`、`inspector_below_stage=true`、`toolbar_overflows_viewport=false`；同服务桌面 focus-switch 回归 artifact `artifacts/ui-graph-layout-desktop-after-mobile-55702.json` 也通过。
  - 当前验证（减少动态门禁）：知识图谱现在尊重 `prefers-reduced-motion: reduce`，首屏会同步收敛为静态布局并保持 `requestAnimationFrame` 停止，状态栏显示“减少动态 · 静态布局”，用户仍可通过“继续动态”显式开启运动；`scripts/ui_graph_layout_acceptance.py` 新增 `--emulate-reduced-motion` / `--expect-reduced-motion`，记录 `reduced_motion` 并检查媒体偏好、图谱状态、RAF、状态文案和动态 opt-in 按钮。当前代码在 `AI_QUANT_PORT=55703` 隔离 SQLite 服务 seed 后，AAPL reduced-motion 验收通过，artifact `artifacts/ui-graph-layout-reduced-motion-55703.json`，初始图 28 节点 / 79 边 / 4 个社区标签，`reduced_motion.matches=true`、`dynamic=false`、`frame_id=0`。
  - 当前验证（非颜色冗余编码门禁）：节点类型不再只依赖颜色区分；每个 SVG 节点中心显示稳定类型短码并带 `data-type-code`，legend 同步显示短码和类型名，节点 `aria-label` 明确语义类型。`scripts/ui_graph_layout_acceptance.py` 新增 `--expect-redundant-node-encoding`，记录 `encoded_node_types`、`legend_encoded_types` 和 `legend_encodings`，当可见节点类型缺少节点短码或 legend 短码时失败。当前代码在 `AI_QUANT_PORT=55704` 隔离 SQLite 服务 seed 后，AAPL 非颜色冗余编码验收通过，artifact `artifacts/ui-graph-layout-redundant-encoding-55704.json`，初始图 28 节点 / 79 边 / 4 个社区标签，可见类型 issuer/event/research/industry/evidence/security 的节点短码和 legend 短码均覆盖，缺失列表为空。
  - 当前验证（图谱输出转义）：图谱 SVG 的边 id/source/target、边标签、节点 id/label、社区 key/label/quality、路径步骤、邻居列表、图例和节点 meta badge 中的节点/边/类型/市场文本、交互 `data-node-id` 统一走 `escapeHtml()` 后再写入 `innerHTML`，防止特殊字符或 raw 文本破坏节点/边/社区标签和路径/邻居交互。
  - 当前验证（大图 DOM 预算门）：`scripts/ui_graph_layout_acceptance.py` 新增 `--max-visible-nodes` / `--max-visible-links` / `--max-link-label-dom-count`，可在 `--expect-performance-mode large` 场景强制可见 SVG 节点、边和边标签 DOM 不超过预算，并把失败时的 `render_stats` 写进 artifact；前端大图模式只为焦点/选中/路径相关边创建 `<text class="graph-link-label">`，路径刷新会在大图模式触发重绘以补齐路径边标签，且 `renderStats.linkLabelDomCount` 作为运行态和验收 artifact 的同一指标来源；高性能状态栏同步显示可见/全量节点、边和标签 DOM 数，避免全局大图回退成把上千节点/边/标签直接塞进 DOM，同时不牺牲路径阅读。
  ...
  - 验收：`python3 scripts/ui_static_check.py`；`python3 -m py_compile app/*.py tests/*.py scripts/*.py`；`python3 scripts/check_handoffs.py`；`git diff --check`；headless Chromium 默认可见文本探针；`python3 scripts/ui_graph_layout_acceptance.py http://127.0.0.1:55552 --symbol AAPL --scope local --min-nodes 26 --min-links 60 --min-community-labels 3 --min-visible-communities 3 --min-community-spread-ratio 0.12 --min-industry-nodes 5 --min-raw-knowledge-nodes 5 --min-visible-knowledge-types 2 --max-overlap-pairs 12 --max-near-edge-nodes 2 --forbid-filter-chip "证券:" --check-focus-switch --output artifacts/ui-graph-layout-acceptance-label-cleanup.json --timeout 45`；生产级非 seed 展示验收在同命令追加 `--require-non-seed-readiness`；大图高性能验收在 `--expect-performance-mode large` 场景追加 `--max-visible-nodes` / `--max-visible-links` / `--max-link-label-dom-count`；减少动态验收追加 `--emulate-reduced-motion --expect-reduced-motion`；非颜色冗余编码验收追加 `--expect-redundant-node-encoding`；`python3 scripts/ui_graph_relationship_filter_acceptance.py http://127.0.0.1:55550 --output artifacts/ui-graph-relationship-filter-acceptance-smoke-2.json --timeout 60`。
...
- `DONE` T-568 图谱质量验收、缺口看板与真实关系增强入口
  ...
  - **已完成（来源动作模型收敛）**：新增 `app/service_modules/graph_source_actions.py`，将 `company_event/company_relationship/document/evidence/shareholder_holding/research_report/viewpoint` 的 action、endpoint、来源字段要求和 usage boundary 从质量中心、增厚 runner 的重复内联字典中抽出；质量中心 `enhancement_actions` 和 runner `layer_action_plan/source_input_queue` 共用同一来源定义，避免后续图谱数据补齐规则漂移。
  ...
  - **已完成（图谱页展示质量门禁）**：知识图谱页新增“检查展示质量”入口，按当前 issuer/symbol 只读调用 `/api/graph/quality-center`，把重复展示标签、原始 ID 泄漏、重复展示边和结构门禁失败项直接渲染到图谱页；来源队列仍保留为数据补齐入口，展示质量和数据密度缺口不再混在一个摘要里。
  - **已完成（展示失败处理动作）**：`quality_gate.remediation_actions` 将图谱展示质量失败路由成明确 dry-run 动作：层/密度/结构问题进入 `/api/graph/enrichment-runner` 来源队列，重复标签/重复展示边进入 `/api/company-database/quality/reconcile` 质量归并预览，raw label 泄漏回到只读质量中心标签模型检查；知识图谱页会在每条失败行展示对应处理动作和 endpoint。
...
- `DONE` T-569 图谱真实事件与关系批量增厚
  ...
  - **已完成（来源输入队列 UI 化）**：公司情报后台维护新增“预览图谱来源队列”，调用 `/api/graph/enrichment-runner` 的 dry-run 来源层计划，把 `source_input_queue` 渲染为队列状态、层数、待补动作、去重标的和分层表格，展示 endpoint、`required_source_fields` 与样本标的；该入口只生成本地/公开/用户提供材料的操作清单，不导入数据、不自动事实提升、不接券商、不触发真实交易。
...
- `DONE` T-566 Obsidian 式可探索知识网络
  ...
  - **已完成（继续推进）**：知识图谱新增“复制视图链接”，把当前 issuer/security、焦点、选中节点、局部/全局范围、深度、搜索词、路径锚点、探索轨迹、焦点历史、展开节点和缩放平移编码到 `#graph=` URL；从分享链接进入 `/ui` 时会自动恢复图谱状态并载入对应公司情报页。
  - **已完成（继续推进验收）**：`scripts/ui_graph_layout_acceptance.py` 新增 `--check-share-view`，在浏览器中生成分享链接、解码 URL 状态、清空当前图谱状态后重新恢复，并断言焦点、选中节点、筛选、搜索词、轨迹和恢复状态一致；当前代码在 `AI_QUANT_PORT=55705` 的本地 SQLite fixture 上通过，artifact 为 `artifacts/ui-graph-layout-share-view-55705.json`。
  - **已完成（继续推进）**：知识图谱可见边新增方向箭头和 `aria-label` / `data-direction-label`，把“source -> target · 关系名”直接编码到图上，减少用户只能从表格或 inspector 推断关系方向的阅读成本。
  - **已完成（继续推进验收）**：`scripts/ui_graph_layout_acceptance.py` 新增 `--expect-directed-edges`，在浏览器中检查每条可见边都有 `graphArrowMarker`、source/target 和方向标签；当前代码在 `AI_QUANT_PORT=55706` 的本地 SQLite fixture 上通过，49 条可见边全部有方向编码，artifact 为 `artifacts/ui-graph-layout-directed-edges-55706.json`。
  - **已完成（继续推进）**：知识图谱可见边新增强度编码，综合现有 `link.weight` 与 `meta.confidence/score/weight` 映射到 SVG `stroke-width`，并在 `data-weight`、`data-confidence`、`data-strength` 和方向 `aria-label` 中保留可读强度/置信度，避免用户必须打开详情才能判断关系强弱。
  - **已完成（继续推进验收）**：`scripts/ui_graph_layout_acceptance.py` 新增 `--expect-weighted-edges`，在浏览器中检查每条可见边都有强度线宽和强度标签，并要求多条边时出现至少两个强度桶；当前代码在 `AI_QUANT_PORT=55707` 的本地 SQLite fixture 上通过，49 条可见边全部有强度编码且出现 15 个强度桶，artifact 为 `artifacts/ui-graph-layout-weighted-edges-55707.json`。
  - **已完成（继续推进）**：知识图谱可见边升级为可直接选择的 SVG 控件，支持鼠标点击和 Enter/Space 键盘选择；选中关系会清空节点选择、突出当前边和两端节点，并在右侧 inspector 展示关系类型、强度、置信度、权重、起点/终点按钮和关系追溯，减少用户只能从节点相邻列表反推边含义的阅读成本。
  - **已完成（继续推进验收）**：`scripts/ui_graph_layout_acceptance.py` 新增 `--check-link-selection`，在真实浏览器中点击首条可见边并断言 `selectedLinkId`、`is-selected-link`、键盘可聚焦、关系 inspector、强度 meta 和两端节点按钮都可用；当前代码在 `AI_QUANT_PORT=55708` 的本地 SQLite fixture 上通过，artifact 为 `artifacts/ui-graph-layout-link-selection-55708.json`。
  ...
  - 验收：`python3 -m unittest tests.test_system.SystemServiceTests.test_obsidian_knowledge_graph_seed_creates_multi_dimension_network tests.test_system.SystemServiceTests.test_graph_knowledge_network_readiness_flags_real_data_gaps_and_seed_dependency`、`python3 scripts/backfill_knowledge_network_evidence_links.py http://127.0.0.1:55549 --issuer-id issuer_aapl --limit 10 --execute --output artifacts/knowledge-network-evidence-link-backfill-executed.json --timeout 10`、`python3 scripts/ui_graph_layout_acceptance.py http://127.0.0.1:55549 --symbol AAPL --scope local --min-nodes 28 --min-links 64 --min-community-labels 3 --min-visible-communities 4 --min-industry-nodes 5 --min-raw-knowledge-nodes 8 --min-visible-knowledge-types 2 --max-overlap-pairs 14 --max-near-edge-nodes 2 --forbid-filter-chip "证券:" --check-focus-switch --output artifacts/ui-graph-layout-acceptance-evidence-links.json --timeout 45`、`python3 scripts/ui_graph_layout_acceptance.py http://127.0.0.1:55705 --symbol AAPL --scope local --min-nodes 21 --min-links 49 --min-community-labels 3 --min-visible-communities 3 --min-community-spread-ratio 0.12 --min-industry-nodes 4 --min-visible-knowledge-types 1 --max-overlap-pairs 14 --max-near-edge-nodes 2 --forbid-filter-chip "证券:" --check-share-view --skip-readiness-probe --output artifacts/ui-graph-layout-share-view-55705.json --timeout 60`、`python3 scripts/ui_graph_layout_acceptance.py http://127.0.0.1:55706 --symbol AAPL --scope local --min-nodes 21 --min-links 49 --min-community-labels 3 --min-visible-communities 3 --min-community-spread-ratio 0.12 --min-industry-nodes 4 --min-visible-knowledge-types 1 --max-overlap-pairs 14 --max-near-edge-nodes 2 --forbid-filter-chip "证券:" --expect-directed-edges --skip-readiness-probe --output artifacts/ui-graph-layout-directed-edges-55706.json --timeout 60`、`python3 scripts/ui_graph_layout_acceptance.py http://127.0.0.1:55707 --symbol AAPL --scope local --min-nodes 21 --min-links 49 --min-community-labels 3 --min-visible-communities 3 --min-community-spread-ratio 0.12 --min-industry-nodes 4 --min-visible-knowledge-types 1 --max-overlap-pairs 14 --max-near-edge-nodes 2 --forbid-filter-chip "证券:" --expect-directed-edges --expect-weighted-edges --skip-readiness-probe --output artifacts/ui-graph-layout-weighted-edges-55707.json --timeout 60`、`python3 scripts/ui_graph_layout_acceptance.py http://127.0.0.1:55708 --symbol AAPL --scope local --min-nodes 21 --min-links 49 --min-community-labels 3 --min-visible-communities 3 --min-community-spread-ratio 0.12 --min-industry-nodes 4 --min-visible-knowledge-types 1 --max-overlap-pairs 14 --max-near-edge-nodes 2 --forbid-filter-chip "证券:" --expect-directed-edges --expect-weighted-edges --check-link-selection --skip-readiness-probe --output artifacts/ui-graph-layout-link-selection-55708.json --timeout 60`、`python3 scripts/ui_static_check.py`、`python3 scripts/check_handoffs.py`、`git diff --check`。
...
- （已归档，非当前产品主路线）T-404 生产级状态库、对象存储和检索适配 — 已归档至下方“## 非目标 / 运维附录（非本机组织级生产）”，非当前产品主路线
...
- （已归档，非当前产品主路线）T-406 三市场主体页和知识图谱生产化 — 已归档至下方“## 非目标 / 运维附录（非本机组织级生产）”，非当前产品主路线
...
- （已归档，非当前产品主路线）T-407 CEO Dashboard 与 UI 图对齐验收 — 已归档至下方“## 非目标 / 运维附录（非本机组织级生产）”，非当前产品主路线
- （已归档，非当前产品主路线）T-408 月报/回放生产化和真实绩效归因 — 已归档至下方“## 非目标 / 运维附录（非本机组织级生产）”，非当前产品主路线
- （已归档，非当前产品主路线）T-409 Black-Litterman、风险预算和组合约束原型 — 已归档至下方“## 非目标 / 运维附录（非本机组织级生产）”，非当前产品主路线
...
- （已归档，非当前产品主路线）T-411 生产监控、告警和事故闭环 — 已归档至下方“## 非目标 / 运维附录（非本机组织级生产）”，非当前产品主路线
- （已归档，非当前产品主路线）T-412 生产部署 runbook 与验收清单 — 已归档至下方“## 非目标 / 运维附录（非本机组织级生产）”，非当前产品主路线
## 观点与研究资产附录 / M8
...
- （已归档，非当前产品主路线）T-418 大模型 / Agent 工作流生产化 — 已归档至下方“## 非目标 / 运维附录（非本机组织级生产）”，非当前产品主路线
## 运维/非本机发布附录 / M9 生产基础设施与治理

- （已归档，非当前产品主路线）T-419 图谱 / 向量 / 语义检索生产化 — 已归档至下方“## 非目标 / 运维附录（非本机组织级生产）”，非当前产品主路线
- （已归档，非当前产品主路线）T-420 任务编排、血缘和模型治理 — 已归档至下方“## 非目标 / 运维附录（非本机组织级生产）”，非当前产品主路线
- （已归档，非当前产品主路线）T-421 安全、密钥和权限生产化 — 已归档至下方“## 非目标 / 运维附录（非本机组织级生产）”，非当前产品主路线
