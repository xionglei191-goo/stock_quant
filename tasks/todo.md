...
- `DONE` T-566 Obsidian 标准可探索知识网络升级
  ...
  - 当前验证（路径旁支阅读质量）：路径面板 next-hop 从“当前节点前 3 条边”改为 `graphPathNextHops()`，只展示当前可见节点，并按知识节点/研究/事件/证据优先级、边权和语义价值排序；浏览器验收 artifact 记录 `next_hop_node_types`，避免路径探索入口退回低价值或不可见旁支。
